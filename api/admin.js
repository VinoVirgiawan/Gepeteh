...
function parseCookies(h) {
  ...
  h.split(';').forEach(s => { const p = s.trim().split('='); const k = p[0]; c[k] = decodeURIComponent(p.slice(1).join('=')); }); return c;
}
...
<head>
  ...
  <title>CheatGPT Admin - Login</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .box{background:rgba(18,18,26,.85);backdrop-filter:blur(20px);border:1px solid #1e1e2e;border-radius:20px;padding:48px 40px;width:100%;max-width:420px}
    .box h1{text-align:center;font-size:28px;margin-bottom:8px;background:linear-gradient(135deg,#6c5ce7,#a29bfe);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
    .box .sub{text-align:center;color:#888;margin-bottom:32px;font-size:14px}
    .box .logo{text-align:center;font-size:48px;margin-bottom:16px}
    .fg{margin-bottom:20px}
    .fg label{display:block;font-size:13px;color:#888;margin-bottom:6px}
    ...
    .btn{width:100%;padding:13px;border:none;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;background:linear-gradient(135deg,#6c5ce7,#8b5cf6);color:#fff}
...
<body>
  <div class="box">
    <div class="logo">&#x1F510;</div>
    <h1>CheatGPT Admin</h1>
    <p class="sub">License Key Management</p>
    __ERR__
    <form method="POST" action="/api/admin?do=login">
...
function loginPage(err) {
  return LOGIN_PAGE.replace('__ERR__', err ? '<div class="em">&#x26A0;&#xFE0F; Invalid username or password</div>' : '');
}
...
module.exports = async (req, res) => {
  ...
  // API actions not logged in → JSON 401
  if (act && !logged) {
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    return res.status(401).json({ ok: false, error: 'Not logged in' });
  }

  // API actions (logged in)
  if (act && logged) {
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    const db = loadDB();
    ...
    if (act === 'history') {
      return res.json({ ok: true, history: (db.history || []).slice(-100).reverse() });
    }
    ...
    if (act === 'delete') {
      const b = await readBody(req);
      if (db.keys[b.key]) { delete db.keys[b.key]; saveDB(db); return res.json({ ok: true }); }
      return res.json({ ok: false, error: 'Key not found' });
    }

    if (act === 'toggle') {
      const b = await readBody(req);
      if (db.keys[b.key]) { db.keys[b.key].is_active = !db.keys[b.key].is_active; saveDB(db); return res.json({ ok: true, is_active: db.keys[b.key].is_active }); }
      return res.json({ ok: false, error: 'Key not found' });
    }
    ...
    if (act === 'devices') {
      const b = await readBody(req);
      if (db.keys[b.key]) return res.json({ ok: true, devices: db.keys[b.key].devices || {} });
      return res.json({ ok: false, error: 'Key not found' });
    }
  ...
  // Not logged in + no action = show login page
  if (!logged) {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.status(200).send(loginPage(0));
  }

  // Logged in + no action = redirect to dashboard at /
  return res.redirect(302, '/');
};
